...
const refresh = (fileNames, mac, cdn) => {
  ...
  //每次只能包含100个cdn链接
  return Promise.all(chunk(fileNames, MAX_SINGLE_REFRESH).map(chunkFiles => _refresh(chunkFiles.map(name => _join(cdn, name)))));
};
...
const deleteFiles = (bucket, mac, filter = () => true) => {
  ...
  return getAllFiles().then(files => {
    ...
    total = files.length;
    tip({ total, type });
    return Promise.all(chunk(files, MAX_SINGLE_DELETE).map(chunkFiles => _deleteChunkFiles(chunkFiles.map(file => qiniu.rs.deleteOp(bucket, file.key)))));
  });
};
...
module.exports = class QiniuPlugin {
  ...
  apply(compiler) {
    compiler.plugin('after-emit', (compilation, callback) => {
      ...
      const bucket = _options.bucket,
            accessKey = _options.accessKey,
            secretKey = _options.secretKey;
      var _options$chunkSize = _options.chunkSize;
      const chunkSize = _options$chunkSize === undefined ? 20 : _options$chunkSize,
            exclude = _options.exclude,
            clean = _options.clean,
            refreshCDN = _options.refreshCDN;
      var _options$cleanExclude = _options.cleanExclude;
      const cleanExclude = _options$cleanExclude === undefined ? () => false : _options$cleanExclude;
      var _options$refreshFilte = _options.refreshFilter;
      const refreshFilter = _options$refreshFilte === undefined ? () => true : _options$refreshFilte;
      ...
      const uploadChunk = err => {
        ...
        if (chunkFiles.length > 0) {
          return Promise.all(chunkFiles.map(fileName => upload(fileName))).then(() => uploadChunk()).catch(uploadChunk);
        } else {
